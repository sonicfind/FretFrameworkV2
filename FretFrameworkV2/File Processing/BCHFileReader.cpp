...
{
	uint32_t headerSize = extract<uint32_t, false>();
	uint32_t trackSize = extract<uint32_t, false>();
	m_next = m_currentPosition + headerSize;
	m_nextTracks.push_back(m_next + trackSize);

	if (m_nextTracks.back() > getEndOfFile())
...
bool BCHFileReader::validateTrack(const char(&str)[5])
{
	if (!checkTag(str))
		return false;
...
{
	ChartEvent type = (ChartEvent)extract<unsigned char, false>();
