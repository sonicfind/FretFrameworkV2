...
MD5::MD5(const char* const inputBuffer, size_t const length)
{
    ...
    {
        evaluateBlock(inputBuffer + index);
...
static void Calculate(uint32_t(&buffer)[4], const uint32_t(&_integerValues)[16])
{
    ...
    for (int j = 0; j < 16;)
    {
        for (int i = 0; i < 4; ++i, ++j)
        {
            uint32_t value = _integerValues[j] + buffer[3];
            if constexpr (__ROUND == 0)
                value += (buffer[0] & buffer[1]) | (~buffer[0] & buffer[2]);
            else if constexpr (__ROUND == 1)
                value += (buffer[2] & buffer[0]) | (~buffer[2] & buffer[1]);
            else if constexpr (__ROUND == 2)
                value += buffer[0] ^ buffer[1] ^ buffer[2];
            else
                value += buffer[1] ^ (buffer[0] | ~buffer[2]);

            buffer[3] = buffer[2];
            buffer[2] = buffer[1];
            buffer[1] = buffer[0];
            buffer[0] += _rotl(value, SHIFTS[i]);
        }
    }
}
...
void MD5::evaluateBlock(const void* _Ptr)
{
    ...
    uint32_t buffer[4] = { m_value[1], m_value[2], m_value[3], m_value[0] };
    Calculate<0>(buffer,
                    { block[0]  + INTEGERTABLE[0],
